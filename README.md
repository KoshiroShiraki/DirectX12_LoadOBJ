...
〇アピールポイント
ゲームエンジンおよびツール開発業務に携わりたいことを企業にアピールする目的で本作品を制作しました。
アピールポイントはモデルファイルのパースと表示、シャドウイングやライティングなど、ゲームエンジン上ではすでに実装されているものを自分で実装したことです。
ソースコードで特にみていただきたいのは
"\ObjectViewer\ObjectViewer_D3D12\DX12ObjectFormatOBJ.cpp"内のLoadVertexFromOBJFile関数です。
現在はテキストファイルから文字列で取得し、ライブラリ関数を用いて数値にする、という方法をとっていますがどうしても容量の大きいファイルになると読み込み速度がネックとなってしまいます。既存の3Dモデル表示可能アプリ(Windowsの3DビューアーやBlender)と比較しても数秒の差が生じます。ソースコードを見て何かアドバイスがあればいただきたいです。

もしお時間あればでよろしいので、コードの全体的な可読性の高さや変数名のつけ方など気になる点がありましたらご指摘いただけると幸いです。
